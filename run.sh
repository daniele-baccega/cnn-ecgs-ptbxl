...
: '
  ...
  Inputs:
      ...
      -kslc or  --kernel_size_last_conv:  kernel size of the last convolutional inter-leads layer
      -oaf2 or  --out_act_fun_2_classes:  output activation function for the two classes classification task
      -rp   or  --rpeak:                  R-peak pose normalization data augmentation technique
      -df   or  --different_filters:      use different filters for each lead with the 2D model
      -nffc or  --num_filters_first_conv: number of filters in the first convolutional layer
      -odm  or  --one_d_model:            use the 1D model instead of the 2D model
      -jstd or  --jitter_std              interval used to extract a uniform standard deviation for the random jitter data augmentation technique
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -rp|--rpeak)
      ...
      shift
      ;;
    -df|--different_filters)
      DIFFERENT_FILTERS="--different_filters"
      shift
      ;;
    -nffc|--num_filters_first_conv)
      NUM_FILTERS_FIRST_CONV="$2"
      shift
      shift
      ;;
    -odm|--one_d_model)
      ONE_D_MODEL="--one_d_model"
      shift
      ;;
    -jstd|--jitter_std)
      JITTER_STD="$2 $3"
    ...
    -h|--help)
      ...
      printf "        -nc   or  --num_classes:             specific classification task (binary classification, five-class multi-label classification or\n"
  	  printf "                                             twenty-class multi-label classification or hierarchical twenty-four-class multi-label classification)\n"
      ...
      printf "        -rp   or  --rpeak:                   R-peak pose normalization data augmentation technique\n"
      printf "        -df   or  --different_filters:       use different filters for each lead with the 2D model\n"
      printf "        -nffc or  --num_filters_first_conv:  number of filters in the first convolutional layer\n"
      printf "        -odm  or  --one_d_model:             use the 1D model instead of the 2D model\n"
      printf "        -jstd or  --jitter_std:              interval used to extract a uniform standard deviation for the random jitter data augmentation technique\n"
      exit 1
      ;;
    -*|--*)
...
if [ $NUM_CLASSES -eq 2 ] || [ $NUM_CLASSES -eq 5 ] || [ $NUM_CLASSES -eq 20 ] || [ $NUM_CLASSES -eq 24 ]; then
    ...
    mkdir checkpoints
		...
		python3 main.py --seed $SEED --num_classes $NUM_CLASSES --dataset $DATASET --epochs $EPOCHS --batch_size $BATCH_SIZE --init_learning_rate $INIT_LEARNING_RATE \
                    ...
                    --num_filters_first_conv $NUM_FILTERS_FIRST_CONV --jitter_std $JITTER_STD $RPEAK $DIFFERENT_FILTERS $ONE_D_MODEL
	  
    cd ..
  fi
else
	echo "The number of classes to use must be equals to 2, 5, 20 or 24."
  exit 1
fi