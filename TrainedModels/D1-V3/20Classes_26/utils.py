...
def save_and_display_gradcam(img_array, heatmap, j, selected_leads_name, explainability_dir, cam_path="cam.jpg", alpha=0.7):
    ...
    for i in range(len(selected_leads_name)):
      heatmap_colors                = np.array([(x[0]/255, x[1]/255, x[2]/255, alpha) for x in jet_heatmap[i, :, :]])

      fig                           = plt.figure(figsize=(15, 10))
      ax                            = fig.add_subplot(1, 1, 1)
      ax.set_title(selected_leads_name)
      ax.plot(img_array[i, :], 'k-', alpha=0.3)
      ax.scatter(range(0, img_array.shape[1]), img_array[i, :], c=heatmap_colors, s=8)
      plt.colorbar(cm.ScalarMappable(cmap=jet))
      plt.grid(False)
      plt.savefig(explainability_dir + 'lead_' + selected_leads_name[i] + '_' + str(j) + '.png', dpi=512)
      plt.close()
