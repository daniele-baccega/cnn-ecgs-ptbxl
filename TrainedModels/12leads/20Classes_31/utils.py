...
def get_classes_dic_and_output_activation_function(num_classes):
  ...
  return classes_dic, output_act_fun

#  Function useful to process and save raw data (or load it)
def process_raw_data(data_dir_exists, num_classes, classes_dic, sampling_rate, path, selected_leads_indeces, likelihood_threshold=100, train_folds=[1, 2, 3, 4, 5, 6, 7, 8], val_fold=9, test_fold=10):
  X                                     = None
  Y                                     = None
  sample_weights                        = None
...
## Utility functions (explainability)
#  Function useful to create the heatmap for the explainability part
def make_gradcam_heatmap(img_array, model, last_conv_layer_name, pred_index=None):
    ...
    with tf.GradientTape() as tape:
      ...
      print("Real prediction:", tensorflow.argmax(preds[0]))
...
#  Function useful to visualize the heatmap on the ECG for the explainability part
def save_and_display_gradcam(img_array, heatmap, cam_path="cam.jpg", alpha=0.7):
    ...
    img_array                       = img_array[:, 6:551, :]

    for i in range(12):
      heatmap_colors                = np.array([(x[0]/255, x[1]/255, x[2]/255, alpha) for x in jet_heatmap[i, 6:551, :]])

      fig                           = plt.figure(figsize=(15, 10))
      ax                            = fig.add_subplot(1, 1, 1)
      ax.set_title(lead_list[i])
      ax.plot(img_array[i, :, 0], 'k-', alpha=0.3)
      ax.scatter(range(0, img_array.shape[1]), img_array[i, :, 0], c=heatmap_colors, s=8)
      plt.colorbar(cm.ScalarMappable(cmap=jet))
      plt.grid(False)
      plt.savefig('lead_' + lead_list[i] + '.png', dpi=512)