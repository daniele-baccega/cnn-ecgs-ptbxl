...
if args.num_classes not in [2, 5, 20]:
  print("The number of classes must be equals to 2, 5 or 20.")
  exit()
...
history_df['type'] = np.concatenate((["Train " + str('%.3f'%(history['loss'][min_train_loss_idx])) + "@" + str((min_train_loss_idx + 1)) for x in epochs], ["Validation " + str('%.3f'%(history['val_loss'][min_val_loss_idx])) + "@" + str((min_val_loss_idx + 1)) for x in epochs], ["Test " + str('%.3f'%(history['test_loss'][min_test_loss_idx])) + "@" + str((min_test_loss_idx + 1)) for x in epochs]))
...
history_df['type'] = np.concatenate((["Train " + str('%.3f'%(history['binary_accuracy'][max_train_accuracy_idx])) + "@" + str((max_train_accuracy_idx + 1)) for x in epochs], ["Validation " + str('%.3f'%(history['val_binary_accuracy'][max_val_accuracy_idx])) + "@" + str((max_val_accuracy_idx + 1)) for x in epochs], ["Test " + str('%.3f'%(history['test_binary_accuracy'][max_test_accuracy_idx])) + "@" + str((max_test_accuracy_idx + 1)) for x in epochs]))
...
for i in range(num_classes_iterations):
	...
	print("Accuracy class {0}: {1:0.3f}".format(classes_dic[i], accuracy[i]))
