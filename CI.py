'''
	...
	Compute the averages and the 95% CIs for the AUC, sensitivity and specificity over the PTX-XL dataset test set.
	We compute sensitivity and specificity using the best threshold for each diagnosis.
...
for path in paths:
	...
	for count in range(1, runs):
		...
		fn 										= mcm[:, 1, 0]
		fp 										= mcm[:, 0, 1]
		sensitivity_local[0:num_classes]		= tp / (tp + fn)
		specificity_local[0:num_classes]		= tn / (tn + fp)
		sensitivity_local[num_classes]			= np.mean(sensitivity_local[0:num_classes])
		specificity_local[num_classes]			= np.mean(specificity_local[0:num_classes])
