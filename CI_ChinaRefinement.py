...
paths 										= ["ChinaRefinement/D1", "ChinaRefinement/D1-D2", "ChinaRefinement/12leads"]
...
pd.DataFrame(global_roc_auc_mean, index=classes_dic_name_id.keys(), columns=paths).to_csv("mean_AUC_ChinaRefinement.csv", float_format='%.2f')
pd.DataFrame(global_roc_auc_left, index=classes_dic_name_id.keys(), columns=paths).to_csv("left_AUC_ChinaRefinement.csv", float_format='%.2f')
pd.DataFrame(global_roc_auc_right, index=classes_dic_name_id.keys(), columns=paths).to_csv("right_AUC_ChinaRefinement.csv", float_format='%.2f')
